...
- Added server-handler regressions for undecodable payload paths: invalid binary-string message decode should emit the same status-0 malformed envelope, decoded payloads with non-string/blank-string `method` should emit the same explicit missing/invalid-method envelope, whitespace-padded valid methods should dispatch successfully after trim normalization, prototype/constructor traversal attempts (including surrounding-whitespace, exact root `constructor`, `prototype`-segment, nested-segment variants, and inherited prototype method paths like `core.toString`), empty/whitespace-segment method traversal attempts (`core..ping`, `core. ping`) should be rejected as missing handlers, and undecodable `params` payloads should produce status-0 error propagation instead of unhandled throw behavior.

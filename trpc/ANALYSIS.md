...
- Method target resolution now blocks `__proto__`/`prototype`/`constructor` path segments and empty path segments (e.g. `core..ping`) to avoid unsafe or ambiguous traversal while preserving valid caller semantics.

...
- `socketServer.ts`: server-side request handler for `trpc` events and standardized `trpcResponse` envelopes, including explicit invalid-method guards (missing/non-string/blank method values), trimmed-method dispatch for whitespace-tolerant valid calls, strict method-path validation (blocks `__proto__` / `prototype` / `constructor`, empty path segments like `core..ping`, and whitespace-padded segments like `core. ping`), and traversal-time rejection of inherited built-in prototype methods across common JS prototypes (e.g. `core.toString`, `core.list.map`).

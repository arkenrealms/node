...
- `socketServer.ts`: server-side request handler for `trpc` events and standardized `trpcResponse` envelopes, including explicit invalid-method guards (missing/non-string/blank method values), trimmed-method dispatch for whitespace-tolerant valid calls, and strict method-path validation (blocks `__proto__` / `prototype` / `constructor` plus empty path segments like `core..ping`) during method resolution.

...
## Files
- `socketLink.ts`: client link + proxy, callback correlation, request ID collision avoidance, same-tick response safety (register callback before emit), timeout/error handling, strict response-id parsing, safe malformed-response and server-push decode handling, optional `onAny` response handling.

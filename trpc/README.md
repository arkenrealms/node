...
- `socketServer.ts`: server-side request handler for `trpc` events and standardized `trpcResponse` envelopes, including explicit invalid-method guards (missing/non-string/blank method values) and trimmed-method dispatch for whitespace-tolerant valid calls.
